const svg_lisbon_private = d3.select("#map_lisbon_private"),
    width_lisbon_priv = +svg_lisbon.attr("width"),
    height_lisbon_priv = +svg_lisbon.attr("height");
...
let scale_priv = 20000;;
var aux_dict = {};


let promises_priv =[
    d3.csv("../data/Lisboa_priv.csv",function(d) {
        return {
            latitude:d.latitude,
...
function draw_lisbon(lisbon_map) {
    ...
    let data_lisbon = lat_long.filter(function (d) {
        return d.city == "Lisboa" && d.types_room == "Private room";
    })
    ...
    let transla = [width_lisbon_priv / 2, height_lisbon_priv / 2]
    let projection = d3.geoMercator().scale(scale_priv).center(center)
        .translate(transla);

    let path = d3.geoPath().projection(projection);
    var bounds = path.bounds(geojson);
    transla = [width_lisbon_priv - (bounds[0][0] + bounds[1][0]) / 2,
        height_lisbon_priv - (bounds[0][1] + bounds[1][1]) / 2];
