const svg_lisbon_shared = d3.select("#map_lisbon_shared"),
    width_lisbon_shared = +svg_lisbon.attr("width"),
    height_lisbon_shared = +svg_lisbon.attr("height");
...
let promises_shared =[
    d3.csv("../data/Lisboa_sha.csv",function(d) {
        return {
            latitude:d.latitude,
...
function draw_lisbon(lisbon_map) {
    ...
    let data_lisbon = lat_long.filter(function (d) {
        return d.city == "Lisboa" && d.types_room == "Shared room";
    })
    ...
    let transla = [width_lisbon_shared / 2, height_lisbon_shared / 2]
    let projection = d3.geoMercator().scale(scale_sha).center(center)
        .translate(transla);

    let path = d3.geoPath().projection(projection);
    var bounds = path.bounds(geojson);
    transla = [width_lisbon_shared - (bounds[0][0] + bounds[1][0]) / 2,
        height_lisbon_shared - (bounds[0][1] + bounds[1][1]) / 2];
