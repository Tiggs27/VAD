const margin_graf = {top: 100, right: 20, bottom: 100, left: 50},
    width = 960 - margin_graf.left - margin_graf.right,
    height = 400 - margin_graf.top - margin_graf.bottom;

// append the svg object to the body of the page
const svg = d3.select("#map_porto")
    .append("svg")
    .attr("width", width + margin_graf.left + margin_graf.right)
    .attr("height", height + margin_graf.top + margin_graf.bottom)
    .append("g")
    .attr("transform",`translate(${margin_graf.left},${margin_graf.top})`);

let dataReady, x, y, myColor;

//Read the data
d3.csv("../data/Lisboa_preco_analise.csv").then(function(data) {
    //console.log(data)
    // List of groups (here I have one group per column)
    const allGroup = ["Lisboa", "Cascais", "Sintra","Mafra","Alenquer","Vila Franca de Xira", "Cadaval", "Lourinh","Torres Vedras","Oeiras","Amadora","Sobral De Monte Agrao","Arruda Dos Vinhos","Loures","Azambuja","Odivelas"]
