const svg_porto_ho = d3.select("#map_porto_ho"),
    width_porto_ho = +svg_porto.attr("width"),
    height_porto_ho = +svg_porto.attr("height");
...
let promises_ho =[
    d3.csv("../data/Porto_hotel.csv",function(d) {
        return {
            latitude:d.latitude,
...
function draw_lisbon(lisbon_map) {
    ...
    let data_lisbon = lat_long.filter(function (d) {
        return d.city == "Porto" && d.types_room == "Hotel room";
    })
    ...
    let transla = [width_porto_ho / 2, height_porto_ho / 2]
    let projection = d3.geoMercator().scale(scale_ho).center(center)
        .translate(transla);

    let path = d3.geoPath().projection(projection);
    var bounds = path.bounds(geojson);
    transla = [width_porto_ho - (bounds[0][0] + bounds[1][0]) / 2,
        height_porto_ho - (bounds[0][1] + bounds[1][1]) / 2];
